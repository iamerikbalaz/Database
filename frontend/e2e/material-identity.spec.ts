...
test("confirmed rebrand renames real source files and survives backend and worker restart", async ({ page }) => {
  ...
  expect(inventory.inventory.entries.filter((entry: { path: string }) => entry.path.includes("E2E_NEXT_0001_E2E-CONTROLLED-IDENTITY_" ) || entry.path.endsWith("E2E_NEXT_0001_E2E-CONTROLLED-IDENTITY.sbs"))).toHaveLength(4);
