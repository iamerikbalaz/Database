...
async def safe_request_validation_handler(
    ...
    if (is_auth_api_path(path) or path.startswith(("/api/online-categories", "/api/collections"))
            or (path.startswith("/api/materials/") and ("/identity-" in path or "/content" in path))):
