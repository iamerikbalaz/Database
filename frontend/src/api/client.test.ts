...
function serve(overrides: Record<string, unknown> = {}, failed?: string) {
  const responses: Record<string, unknown> = {
    ...
    "/api/companies/10000000-0000-4000-8000-000000000001": companyDto,
    "/api/brands": brands.map(publishedBrandToDto),
    "/api/projects": projects.map(projectToDto),
    "/api/projects/20000000-0000-4000-8000-000000000001": projectDto,
    ...overrides,
  };
  const fetchMock = vi.fn(async (input: string) => {
    if (!(input in responses) || input === failed)
...
describe("API contract", () => {
  it("maps every Company field in both directions, preserving nulls", () => {
    expect(companyDto).toEqual({
      ...
      updated_at: companies[0].updatedAt,
    });
  ...
  it("maps full PublishedBrand including managed counter and timestamps", () => {
    expect(brandDto).toMatchObject({
      company_id: "10000000-0000-4000-8000-000000000001",
      folder_prefix: "SWISSPEARL",
      brand_identifier: "swisspearl",
      next_sequence_number: 1,
      is_active: true,
    });
    expect(publishedBrandFromDto(parsePublishedBrand(brandDto))).toEqual(
      brands[0],
    );
    expect(publishedBrandToDto(publishedBrandFromDto(brandDto))).toEqual(
      brandDto,
    );
  });
  it("maps project number, owner, notes, dates and timestamps", () => {
    expect(projectDto).toMatchObject({
      company_id: "10000000-0000-4000-8000-000000000001",
      project_number: "RWT-2026-018",
      due_date: "2026-09-18",
      notes: projects[0].description,
      status: "IN_PROGRESS",
    });
    expect(projectFromDto(parseProject(projectDto))).toEqual(projects[0]);
    expect(
      projectToDto(
        projectFromDto({ ...projectDto, notes: null, due_date: null }),
      ),
  ...
  it("allows explicitly injected mock data without HTTP", async () => {
    const fetchMock = serve();
    expect(
      await mockApiClient.getCompany("10000000-0000-4000-8000-000000000001"),
    ).toMatchObject({
      name: "Swisspearl",
      brands: [brands[0]],
      projects: [projects[0]],
  ...
  it("assembles CompanyDetail from three endpoints and filters both relations", async () => {
    const fetchMock = serve();
    expect(
      await httpApiClient.getCompany("10000000-0000-4000-8000-000000000001"),
    ).toEqual({
      ...companies[0],
      brands: [brands[0]],
      projects: [projects[0]],
  ...
  it.each([
    "/api/companies/10000000-0000-4000-8000-000000000001",
    "/api/brands",
    "/api/projects",
  ])("rejects detail when %s fails", async (path) => {
    serve({}, path);
    await expect(
      httpApiClient.getCompany("10000000-0000-4000-8000-000000000001"),
    ).rejects.toThrow("404");
  });
  it("rejects malformed related data instead of exposing undefined arrays", async () => {
    ...
    await expect(
      httpApiClient.getCompany("10000000-0000-4000-8000-000000000001"),
    ).rejects.toThrow("Invalid API list");
  });
  it("loads project detail and resolves its company name", async () => {
    serve();
    expect(
      await httpApiClient.getProject("20000000-0000-4000-8000-000000000001"),
    ).toEqual({ ...projects[0], clientName: "Swisspearl" });
  });
  ...
  it("reports missing IDs", async () => {
    ...
    await expect(mockApiClient.getProject("missing")).rejects.toThrow(
      "not found",
    );
  });
});
...
it("preserves empty related lists in CompanyDetail", async () => {
  serve({ "/api/brands": [], "/api/projects": [] });
  expect(await httpApiClient.getCompany(companies[0].id)).toEqual({
    ...companies[0],
    brands: [],
    projects: [],
  });
});
it("validates UUID values and retains nullable API fields", () => {
  expect(() => parseCompany({ ...companyDto, id: "not-a-uuid" })).toThrow(
    "Invalid API UUID",
  );
  expect(() => parseProject({ ...projectDto, company_id: null })).toThrow();
  expect(() => parsePublishedBrand({ ...brandDto, company_id: "c-1" })).toThrow(
    "Invalid API UUID",
  );
  expect(
    parseCompany({ ...companyDto, legal_name: null }).legal_name,
  ).toBeNull();
});
