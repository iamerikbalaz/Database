...
from app.auth.access import ApplicationAccess, ADMIN, CATALOG_MANAGERS, MATERIAL_EDITORS, require_application_access
...
class DomainTestAccess(ApplicationAccess):
    ...
    def check(self, session, roles=None, *, exclusive=False):
        # Resource writes now persist the actor in their immutable audit. Keep this
        # legacy domain-only fixture explicit; real authorization tests use actual
        # sessions and never install this override.
        if roles in (ADMIN, CATALOG_MANAGERS, MATERIAL_EDITORS) and session.get(InternalUser, self.user.id) is None:
